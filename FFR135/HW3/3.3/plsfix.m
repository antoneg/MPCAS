...
%     M = Ms(currentRun);
    ...
    %Init weights range [-1, 1] %% OBS, nope!! and thresholds -1 or 1
...
%                  hTmp = hTmp';
%                 
%                 vLocalField = (hTmp*weights);%-visableThres;
                for j = 1:9
                    
%                     size(hTmp)
%                     size(weights(:,j))
                vLocalField(j) = dot(hTmp,(weights(:,j)));
                end
            ...
            %         deltaHiddenThresholds = ComputeDeltaHiddenThresholds(n,weights,v0,v(:,k),hiddenThres);
            %         deltaVisableThresholds = ComputeDeltaVisableThresholds(n, v0, v(:,k));
        end
        for u = 1:nrOfPats
            weights = weights + deltaWeights(:,:,u);
            %        hiddenThres = hiddenThres +deltaHiddenThresholds;
            %        visableThres = visableThres + deltaVisableThresholds;
